 ...
 * behaviour through attrs so one class serves articles, edits and comments:
...
// null at module-init. We resolve it asynchronously: build the WikiComposer
// subclass the first time it's needed (loading the chunk if necessary) and
// cache it. A static `import ComposerBody` would force webpack to eagerly
// require the chunk and break.
 ...
 * on demand.
 */
export function wikiComposerAvailable(): boolean {
...
/**
 * Open the docked composer with a WikiComposer body. Loads the composer chunk
 * first if needed. Returns true when the real composer is available; false on
 * stripped installs so callers can fall back to an inline editor.
 */
export function openWikiComposer(attrs: any): boolean {
  if (!wikiComposerAvailable()) return false;
 ...
 */
export function wikiComposerPreview(opts: { composing?: boolean; placeholder: any; onclick: () => void }): any {
